...
class TabPanel extends HTMLElement {
  ...
  get section() { return this.querySelector('[data-tabpanel]'); }
}
...
class TabStrip extends HTMLElement {
  ...
  constructor() {
    ...
    this.#shadow.innerHTML = /*html*/`
      ...
      <nav part="tabs" aria-label="${this.getAttribute('label') || 'Tabs'}"></nav>
      <slot></slot>
    `;
  }
  ...
  connectedCallback() {
    ...
    this.#mo.observe(this, {subtree: true, childList: true, attributes: true, attributeFilter: ['id','label']});
  ...
  #renderTabs() {
    const nav = this.#shadow.querySelector('nav');
    nav.innerHTML = this.#panels.map(p => {
      const id = p.id;
  ...
  #syncFromUrl(initial=false) {
    ...
    const next = exists ? hashId : (this.getAttribute('selected') || this.#panels[0].id);
    ...
    for (const p of this.#panels) {
      ...
      if (active) sec.setAttribute('tabindex','0'); else sec.removeAttribute('tabindex');
    }
  ...
  #onNavClick = (e) => {
    const a = e.composedPath().find(el => el?.tagName === 'A' && el.getAttribute('href')?.startsWith('#'));
    if (!a) return;
  ...
  #onNavKeydown = (e) => {
    ...
    const i = links.indexOf(this.#shadow.activeElement || document.activeElement);
    if (i === -1) return;
    e.preventDefault();
    const next = e.key === 'ArrowRight' ? links[(i+1)%links.length] : links[(i-1+links.length)%links.length];
    next?.focus();
  };
}
