...
var PDS2 = __existingPDS__ && typeof __existingPDS__.addEventListener === "function" ? __existingPDS__ : new PDSBase();
if (__globalScope__) {
  __globalScope__[__PDS_SINGLETON_KEY__] = PDS2;
}
...
var __defaultLog = function(level = "log", message, ...data) {
  const isStaticMode = Boolean(PDS2.registry && !PDS2.registry.isLive);
  const debug = (this?.debug || this?.design?.debug || PDS2.debug || false) === true;
  if (isStaticMode) {
    if (!PDS2.debug)
      return;
...
async function start(config) {
  ...
  try {
    ...
    if (mode === "static") {
      ...
      const { startLive: startLive2 } = await import(managerUrl);
      startResult = await startLive2(PDS2, rest, {
