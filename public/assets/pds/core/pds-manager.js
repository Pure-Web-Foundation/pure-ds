...
function buildPresetOmniboxSettings(PDS2, options = {}) {
  const {
    hideCategory = true,
    itemGrid = "45px 1fr",
  ...
  const resolvedTheme = resolveThemePreference(theme ?? PDS2?.theme);
    ...
    categories: {
      [categoryName]: {
        ...
        action: async (selection) => {
          ...
          if (typeof PDS2?.applyLivePreset === "function") {
            await PDS2.applyLivePreset(preset.id || selection.id);
          }
...
async function __attachLiveAPIs(PDS2, { applyResolvedTheme, setupSystemListenerIfNeeded }) {
  ...
  PDS2.configSpec = PDS_DESIGN_CONFIG_SPEC;
  PDS2.configEditorMetadata = PDS_DEFAULT_CONFIG_EDITOR_METADATA;
  PDS2.configFormSchema = PDS_DEFAULT_CONFIG_FORM_SCHEMA;
  PDS2.buildConfigFormSchema = buildDesignConfigFormSchema;
  PDS2.getConfigEditorMetadata = getDesignConfigEditorMetadata;
  ...
  PDS2.buildPresetOmniboxSettings = function(options = {}) {
    return buildPresetOmniboxSettings(PDS2, options);
  };
  PDS2.applyLivePreset = async function(presetId, options = {}) {
    ...
    PDS2.configEditorMetadata = getDesignConfigEditorMetadata(
      normalized.generatorConfig.design
    );
    PDS2.configFormSchema = buildDesignConfigFormSchema(
      normalized.generatorConfig.design
    );
