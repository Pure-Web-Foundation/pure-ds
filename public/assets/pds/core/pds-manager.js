...
</button>`,description:"Button with only an icon, no text label"})),a}queryPatterns(t,e){let a=[],r=this.pds.ontology;return r?.layoutPatterns&&(r.layoutPatterns.forEach(o=>{let i=this.scoreMatch(e,o.name+" "+o.id+" "+(o.description||""));if(i>50){let c=o.selectors?.[0]||`.${o.id}`;a.push({text:`${o.name} - ${o.description||"Layout pattern"}`,value:o.id,icon:"layout",category:"Layout Pattern",score:i,code:`<div class="${c.replace(".","")}">
  <!-- content -->
...
`),e.push(`   /* Backdrop tokens - used for modal dialogs, drawers, overlays */
    ...
    `),e.push(this.#N(t)),`${e.join("")}
`}#N(t){let e=t.primary?.[500]||"#3b82f6",a=t.secondary?.[500]||"#8b5cf6",r=t.accent?.[500]||"#f59e0b";return`
...
`)}),`${e.join("")}
`}#I(t){let e=[`  /* Border Radius */
`];return Object.entries(t).forEach(([a,r])=>{e.push(`  --radius-${a}: ${r};
`)}),`${e.join("")}
`}#U(t){let e=[`  /* Border Widths */
`];return Object.entries(t).forEach(([a,r])=>{e.push(`  --border-width-${a}: ${r};
`)}),`${e.join("")}
`}#O(t){let e=[`  /* Typography */
`];return Object.entries(t).forEach(([a,r])=>{let o=a.replace(/^font/,"").replace(/^(.)/,i=>i.toLowerCase()).replace(/([A-Z])/g,"-$1").toLowerCase();Object.entries(r).forEach(([i,c])=>{let l=i.replace(/([A-Z])/g,"-$1").toLowerCase();e.push(`  --font-${o}-${l}: ${c};
`)})}),`${e.join("")}
`}#H(t){let e=[`  /* Shadows */
...
`)}),`${e.join("")}
`}#Q(t){if(!t?.dark)return"";let e=[],a=(d,s="")=>{Object.entries(d).forEach(([p,g])=>{typeof g=="object"&&g!==null?a(g,`${s}${p}-`):typeof g=="string"&&e.push(`  --color-${s}${p}: ${g};
`)})};Object.entries(t.dark).forEach(([d,s])=>{d!=="surfaceSmart"&&typeof s=="object"&&s!==null&&a(s,`${d}-`)});let r=[];t.dark.surfaceSmart&&(r.push(`  /* Smart Surface Tokens (dark mode, context-aware) */
`),Object.entries(t.dark.surfaceSmart).forEach(([d,s])=>{r.push(`  --surface-${d}-bg: ${s.bg};
`),r.push(`  --surface-${d}-text: ${s.text};
`),r.push(`  --surface-${d}-text-secondary: ${s.textSecondary};
`),r.push(`  --surface-${d}-text-muted: ${s.textMuted};
`),r.push(`  --surface-${d}-icon: ${s.icon};
`),r.push(`  --surface-${d}-icon-subtle: ${s.iconSubtle};
`),r.push(`  --surface-${d}-shadow: ${s.shadow};
`),r.push(`  --surface-${d}-border: ${s.border};
`)}),r.push(`
...
`,i=`  /* Backdrop tokens - dark mode */
  ...
  /* Legacy alias for backwards compatibility */
  --backdrop-background: var(--backdrop-bg);
`,c=this.#Y(t);return`html[data-theme="dark"] {
${[...e,...r,o,i,c].join("")}}
`}#Z(t){if(!t?.dark)return"";let e=[],a=(s,p="")=>{Object.entries(s).forEach(([g,h])=>{typeof h=="object"&&h!==null?a(h,`${p}${g}-`):typeof h=="string"&&e.push(`    --color-${p}${g}: ${h};
`)})};Object.entries(t.dark).forEach(([s,p])=>{s!=="surfaceSmart"&&typeof p=="object"&&p!==null&&a(p,`${s}-`)});let r=[];t.dark.surfaceSmart&&(r.push(`    /* Smart Surface Tokens (dark mode, context-aware) */
`),Object.entries(t.dark.surfaceSmart).forEach(([s,p])=>{r.push(`    --surface-${s}-bg: ${p.bg};
`),r.push(`    --surface-${s}-text: ${p.text};
`),r.push(`    --surface-${s}-text-secondary: ${p.textSecondary};
`),r.push(`    --surface-${s}-text-muted: ${p.textMuted};
`),r.push(`    --surface-${s}-icon: ${p.icon};
`),r.push(`    --surface-${s}-icon-subtle: ${p.iconSubtle};
`),r.push(`    --surface-${s}-shadow: ${p.shadow};
`),r.push(`    --surface-${s}-border: ${p.border};
`)}),r.push(`
...
`,`    --color-input-disabled-text: var(--color-gray-600);
`,`    --color-code-bg: var(--color-gray-800);
`,...o].join(""),c=`    /* Backdrop tokens - dark mode */
    --backdrop-bg: linear-gradient(
        135deg,
        rgba(0, 0, 0, 0.6),
        rgba(0, 0, 0, 0.4)
...
${[...e,...r,i,c,l].join("")}       }
`}#J(t){let e=t.dark||t,a=e.primary?.[400]||"#60a5fa",r=e.secondary?.[400]||"#a78bfa",o=e.accent?.[400]||"#fbbf24";return`    /* Mesh Gradient Variables (Dark Mode) */
    ...
    --background-mesh-05: radial-gradient(at 50% 50%, color-mix(in oklab, ${a} 17%, transparent) 0px, transparent 50%),
      ...
      `}#Y(t){let e=t.dark||t,a=e.primary?.[400]||"#60a5fa",r=e.secondary?.[400]||"#a78bfa",o=e.accent?.[400]||"#fbbf24";return`
  ...
  --background-mesh-05: radial-gradient(at 50% 50%, color-mix(in oklab, ${a} 17%, transparent) 0px, transparent 50%),
    ...
    radial-gradient(at 90% 10%, color-mix(in oklab, ${r} 13%, transparent) 0px, transparent 50%),
    radial-gradient(at 50% 90%, color-mix(in oklab, ${o} 10%, transparent) 0px, transparent 50%);
    `}#K(){return`/* Alert dark mode adjustments */
html[data-theme="dark"] {
...
}`}#X(){try{let t=this.options?.design?.options?.backgroundMesh;this.options.debug&&this.options.log?.("debug","backgroundMesh check:",t);let e=Number(t);return!Number.isFinite(e)||e===0?"":`/* Optional background mesh applied from config */
body {
  background: var(--background-mesh-0${Math.max(1,Math.min(5,Math.floor(e)))});
  background-attachment: fixed;
}`}catch(t){return this.options.debug&&this.options.log?.("error","Error in generateBodyBackgroundMeshRule:",t),""}}#ee(){try{return this.options?.design?.options?.liquidGlassEffects?`/* Liquid glass utility (opt-in via options.liquidGlassEffects) */
.liquid-glass {
...
`}#re(){let{layout:t={}}=this.options.design,e=t.breakpoints||{sm:640,md:768,lg:1024,xl:1280};return`/* Semantic HTML Elements (low-specificity via :where()) */
...
`}#ae(){let{gap:t,inputPadding:e,buttonPadding:a,focusRingWidth:r,focusRingOpacity:o,borderWidthThin:i,sectionSpacing:c,buttonMinHeight:l,inputMinHeight:d}=this.options.design,s=e||.75,p=a||1,g=r||3,h=i||1,v=t||1,x=c||2,m=l||44;return`/* Mobile-First Form Styles - Generated from Design Config */
form {
  margin: 0;
  width: 100%;
}
...
input, textarea, select {
  width: 100%;
  min-height: ${d||40}px;
  padding: calc(var(--spacing-1) * ${s}) var(--spacing-4);
...
`}#oe(){let{layout:t={}}=this.options.design,e=t.breakpoints||{sm:640,md:768,lg:1024,xl:1280};return`/* Table Styles - Mobile First */
...
`}#ce(){let{layout:t={},behavior:e={}}=this.options.design;return`/* ============================================================================
   Dialog Primitive
...
`}#le(){let{layout:t={}}=this.options.design;return`/* Tab Strip Component */
...
`}#pe(){let{a11y:t={}}=this.options.design,e=t.minTouchTarget||u.TouchTargetSizes.standard;return`/* Icon System */
...
`}#ge(){let{layout:t={}}=this.options.design,e=t.breakpoints||{sm:640,md:768,lg:1024,xl:1280},a=t.gridSystem||{},r=a.columns||[1,2,3,4,6],o=a.autoFitBreakpoints||{sm:"150px",md:"250px",lg:"350px",xl:"450px"},i=[`
/* ============================================================================
   Layout Utilities
...
`);for(let[c,l]of Object.entries(o))i.push(`.grid-auto-${c} { grid-template-columns: repeat(auto-fit, minmax(${l}, 1fr)); }
`);return i.push(`
/* Gap utilities */
...
`),i.join("")}#me(){return`/* Media Element Utilities */
...
`}#he(){let{layout:t={},a11y:e={}}=this.options.design,a=t.breakpoints||{sm:640,md:768,lg:1024,xl:1280},r=e.minTouchTarget||u.TouchTargetSizes.standard;return`/* Mobile-First Responsive Design */
...
`}#o(t){let e=parseInt(t.slice(1,3),16)/255,a=parseInt(t.slice(3,5),16)/255,r=parseInt(t.slice(5,7),16)/255,o=Math.max(e,a,r),i=Math.min(e,a,r),c,l,d=(o+i)/2;if(o===i)c=l=0;else{let s=o-i;switch(l=d>.5?s/(2-o-i):s/(o+i),o){case e:c=(a-r)/s+(a<r?6:0);break;case a:c=(r-e)/s+2;break;case r:c=(e-a)/s+4;break}c/=6}return{h:c*360,s:l*100,l:d*100}}#t(t,e,a){t=t/360,e=e/100,a=a/100;let r=(d,s,p)=>(p<0&&(p+=1),p>1&&(p-=1),p<1/6?d+(s-d)*6*p:p<1/2?s:p<2/3?d+(s-d)*(2/3-p)*6:d),o,i,c;if(e===0)o=i=c=a;else{let d=a<.5?a*(1+e):a+e-a*e,s=2*a-d;o=r(s,d,t+1/3),i=r(s,d,t),c=r(s,d,t-1/3)}let l=d=>{let s=Math.round(d*255).toString(16);return s.length===1?"0"+s:s};return`#${l(o)}${l(i)}${l(c)}`}getTokens(){return this.tokens}exportCSS(){return this.layeredCSS}#fe(){this.#e={tokens:this.#be(),primitives:this.#ye(),components:this.#ve(),utilities:this.#xe()},this.options.debug&&this.options.log?.("debug","[Generator] Layer sizes:",{tokens:`${(this.#e.tokens.length/1024).toFixed(2)} KB`,primitives:`${(this.#e.primitives.length/1024).toFixed(2)} KB`,components:`${(this.#e.components.length/1024).toFixed(2)} KB`,utilities:`${(this.#e.utilities.length/1024).toFixed(2)} KB`})}#be(){let{colors:t,spacing:e,radius:a,borderWidths:r,typography:o,shadows:i,layout:c,transitions:l,zIndex:d,icons:s}=this.tokens,p=[`@layer tokens {
       :root {
          ${this.#D(t)}
          ${this.#P(e)}
          ${this.#I(a)}
...
`),p.push(this.#Q(t)),p.join("")}#ye(){let{advanced:t={},a11y:e={},layout:a={}}=this.options.design,r=t.tabSize||u.TabSizes.standard,o=e.minTouchTarget||u.TouchTargetSizes.standard,i=a.breakpoints||{sm:640,md:768,lg:1024,xl:1280};return`@layer primitives {
  /* Base HTML reset */
  *, *::before, *::after {
...
<\/script>
See: https://github.com/pure-ds/core#lit-components`):console.warn(`\u26A0\uFE0F PDS component <${m}> not found. Assets may not be installed.`)}else console.error(`\u274C Auto-define error for <${m}>:`,w)},...h,mapper:m=>{if(customElements.get(m))return null;if(typeof r=="function")try{let w=r(m);return w===void 0?p(m):w}catch(w){return console.warn("Custom autoDefine.mapper error; falling back to default:",w?.message||w),p(m)}return p(m)}};s&&(d=new s(x),a.length>0&&typeof s.define=="function"&&await s.define(...a,{baseURL:e,mapper:x.mapper,onError:x.onError}))}return{autoDefiner:d,mergedEnhancers:l}}var Ee=!1,H=null,Ce=!1;async function rt(n,{applyResolvedTheme:t,setupSystemListenerIfNeeded:e}){if(Ee)return;let[a,r,o,i]=await Promise.all([Promise.resolve().then(()=>(Z(),ce)),Promise.resolve().then(()=>(q(),ne)),Promise.resolve().then(()=>(re(),te)),Promise.resolve().then(()=>(Fe(),Me))]),c=a?.default||a?.ontology,l=a?.findComponentForElement,d=r?.enums;H=o?.PDSQuery||o?.default||null,n.ontology=c,n.findComponentForElement=l,n.enums=d,n.common=i||{},n.presets=P,n.applyStyles=function(s){return G(s||F.instance)},n.adoptLayers=function(s,p,g){return de(s,p,g,F.instance)},n.adoptPrimitives=function(s,p){return le(s,p,F.instance)},n.getGenerator=async function(){return F},n.query=async function(s){if(!H){let g=await Promise.resolve().then(()=>(re(),te));H=g?.PDSQuery||g?.default||null}return H?await new H(n).search(s):[]},n._applyResolvedTheme=t,n._setupSystemListenerIfNeeded=e,Object.getOwnPropertyDescriptor(n,"compiled")||Object.defineProperty(n,"compiled",{get(){return n.registry?.isLive&&F.instance?F.instance.compiled:null},enumerable:!0,configurable:!1}),n.setTheme=async function(s,p={}){let{storageKey:g="pure-ds-theme",persist:h=!0}=p;if(!["light","dark","system"].includes(s))throw new Error(`Invalid theme "${s}". Must be "light", "dark", or "system".`);if(typeof window>"u")return s==="system"?"light":s;let v=s;if(s==="system"&&(v=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"),document.documentElement.setAttribute("data-theme",v),h&&localStorage.setItem(g,s),n.registry.isLive&&F.instance)try{let x=F.instance;if(x&&x.configure){let m={...x.config,theme:v};x.configure(m),await G(F.instance)}}catch(x){console.warn("Failed to update styles for new theme:",x)}return n.dispatchEvent(new CustomEvent("pds:theme:changed",{detail:{theme:v,requested:s,source:"programmatic"}})),v},n.preloadCritical=function(s,p={}){if(typeof window>"u"||!document.head||!s)return;let{theme:g,layers:h=["tokens"]}=p;try{let v=g||"light";(g==="system"||!g)&&(v=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"),document.documentElement.setAttribute("data-theme",v);let x=s.design?{...s,theme:v}:{design:s,theme:v},m=new F(x),w=h.map(E=>{try{return m.css?.[E]||""}catch{return""}}).filter(E=>E.trim()).join(`
`);if(w){let E=document.head.querySelector("style[data-pds-preload]");E&&E.remove();let k=document.createElement("style");k.setAttribute("data-pds-preload",""),k.textContent=w,document.head.insertBefore(k,document.head.firstChild)}}catch(v){console.warn("PDS preload failed:",v)}},Ee=!0}function at(n){Ce||typeof document>"u"||(Ce=!0,n.addEventListener("pds:ready",t=>{let e=t.detail?.mode;e&&(document.documentElement.classList.add(`pds-${e}`),e==="live"&&document.documentElement.classList.add("pds-ready"))}))}async function ot(n,t,{emitReady:e,applyResolvedTheme:a,setupSystemListenerIfNeeded:r}){if(!t||typeof t!="object")throw new Error("PDS.start({ mode: 'live', ... }) requires a valid configuration object");if(await rt(n,{applyResolvedTheme:a,setupSystemListenerIfNeeded:r}),at(n),typeof document<"u"&&document.adoptedStyleSheets){let p=`
          html { opacity: 0; }
          html.pds-ready { opacity: 1; transition: opacity 0.3s ease-in; }
        `;try{if(!document.adoptedStyleSheets.some(h=>h._pdsFouc)){let h=new CSSStyleSheet;h.replaceSync(p),h._pdsFouc=!0,document.adoptedStyleSheets=[h,...document.adoptedStyleSheets]}}catch(g){if(console.warn("Constructable stylesheets not supported, using <style> tag fallback:",g),!document.head.querySelector("style[data-pds-fouc]")){let v=document.createElement("style");v.setAttribute("data-pds-fouc",""),v.textContent=p,document.head.insertBefore(v,document.head.firstChild)}}}let o=t.applyGlobalStyles??!0,i=t.manageTheme??!0,c=t.themeStorageKey??"pure-ds-theme",l=t.preloadStyles??!1,d=t.criticalLayers??["tokens","primitives"],s=t&&t.autoDefine||null;try{let{resolvedTheme:p}=ke({manageTheme:i,themeStorageKey:c,applyResolvedTheme:a,setupSystemListenerIfNeeded:r}),g=we(t,{},{presets:P,defaultLog:ie}),h=g.enhancers,{log:v,...x}=g.generatorConfig,m=structuredClone(x);m.log=v,i&&(m.theme=p);let w=new F(m);if(m.design?.typography)try{await fe(m.design.typography)}catch($){m?.log?.("warn","Failed to load some fonts from Google Fonts:",$)}if(l&&typeof window<"u"&&document.head)try{let $=d.map(S=>{try{return w.css?.[S]||""}catch(M){return m?.log?.("warn",`Failed to generate critical CSS for layer "${S}":`,M),""}}).filter(S=>S.trim()).join(`
`);if($){let S=document.head.querySelector("style[data-pds-critical]");S&&S.remove();let M=document.createElement("style");M.setAttribute("data-pds-critical",""),M.textContent=$;let C=document.head.querySelector('meta[charset], meta[name="viewport"]');C?C.parentNode.insertBefore(M,C.nextSibling):document.head.insertBefore(M,document.head.firstChild)}}catch($){m?.log?.("warn","Failed to preload critical styles:",$)}n.registry.setLiveMode(),g.presetInfo?.name?m?.log?.("log",`PDS live with preset "${g.presetInfo.name}"`):m?.log?.("log","PDS live with custom config"),o&&(await G(F.instance),typeof window<"u"&&setTimeout(()=>{let $=document.head.querySelector("style[data-pds-critical]");$&&$.remove();let S=document.head.querySelector("style[data-pds-preload]");S&&S.remove();let M=document.getElementById("pds-runtime-stylesheet");M&&M.remove()},100));let E=Se(t,{resolvePublicAssetURL:me}),k;s&&s.baseURL?k=_(V(s.baseURL,{preferModule:!1})):k=`${E}components/`;let L=null,R=[];try{let $=await $e({autoDefineBaseURL:k,autoDefinePreload:s&&Array.isArray(s.predefine)&&s.predefine||[],autoDefineMapper:s&&typeof s.mapper=="function"&&s.mapper||null,enhancers:h,autoDefineOverrides:s||null,autoDefinePreferModule:!(s&&s.baseURL)},{baseEnhancers:pe});L=$.autoDefiner,R=$.mergedEnhancers||[]}catch($){m?.log?.("error","\u274C Failed to initialize AutoDefiner/Enhancers:",$)}let W=w?.options||m,B=O(t);return n.currentConfig=Object.freeze({mode:"live",...structuredClone(B),design:structuredClone(g.generatorConfig.design),preset:g.generatorConfig.preset,theme:p,enhancers:R}),e({mode:"live",generator:w,config:W,theme:p,autoDefiner:L}),{generator:w,config:W,theme:p,autoDefiner:L}}catch(p){throw n.dispatchEvent(new CustomEvent("pds:error",{detail:{error:p}})),p}}export{ot as startLive};
