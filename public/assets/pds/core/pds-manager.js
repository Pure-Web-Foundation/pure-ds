...
function isUrl(str2) {
  try {
    ...
    if (str2.startsWith("#/"))
      return false;
    const newUrl = new URL(str2, window.location.origin);
...
var presets = {
  ...
  "mobility-app": {
    ...
    colors: {
      ...
      info: "#0e7490",
      // explicit semantic info hue so info-50 remains a subtle tint
      background: "#f6f6f6",
      darkMode: {
        background: "#0f0f0f",
        secondary: "#8a8a8a",
...
var Generator = class _Generator {
  ...
  #generateDarkModeColors(lightColors, backgroundBase = "#ffffff", overrides = {}) {
    ...
    const buildScale = (overrideColor, fallbackScale) => overrideColor ? this.#generateColorScale(overrideColor) : fallbackScale;
    return {
      surface: {
        ...darkSurface,
  ...
  #buildInteractiveRoleTokens(scale = {}, surfaceBg = "#ffffff") {
    ...
    const fillHover = isDarkSurface ? this.#lightenColor(fill, 0.15) : this.#darkenColor(fill, 0.1);
    const fillActive = isDarkSurface ? this.#lightenColor(fill, 0.07) : this.#darkenColor(fill, 0.2);
    ...
    const focusRing = this.#pickReadablePrimaryOnSurface(scale, surfaceBg, 3);
    return {
      fill,
      fillHover,
      fillActive,
  ...
  #buildSurfaceAccentTokens(scale = {}, surfaces = {}) {
    ...
    Object.entries(surfaces).forEach(([surfaceKey, bgColor]) => {
      ...
      const textHover = this.#pickReadablePrimaryOnSurface(scale, bgColor, 5);
      roles[surfaceKey] = {
        text,
        textHover: textHover || text
      };
...
`);
    ...
    Object.entries(colors).forEach(([category, values]) => {
      ...
      if (category === "interactive")
        return;
      if (category === "accentInteractive")
        return;
      if (category === "dangerInteractive")
        return;
      if (category === "successInteractive")
        return;
      if (category === "warningInteractive")
        return;
      if (category === "infoInteractive")
        return;
      if (category === "surfaceAccent")
        return;
      if (category === "surfaceStatus")
        return;
...
`
  ...
  #generateIconStyles() {
    ...
    const buttonPaddingValue = layout.buttonPadding || 1;
...
function validateDesign(designConfig = {}, options = {}) {
  ...
  const MAX_HUE_DRIFT = Number.isFinite(Number(options.maxHueDrift)) ? Number(options.maxHueDrift) : 35;
  const MIN_SAT_FOR_HUE_WARNING = Number.isFinite(
    Number(options.minSaturationForHueWarning)
  ) ? Number(options.minSaturationForHueWarning) : 18;
  ...
  try {
    ...
    if (ENABLE_HUE_DRIFT_WARNINGS) {
      ...
      families.forEach((family) => {
        ...
        if (!lightBase || !darkOverride)
          return;
        const lightHsl = hexToHsl(lightBase);
        const darkHsl = hexToHsl(darkOverride);
        if (!lightHsl || !darkHsl)
          return;
        if (lightHsl.s < MIN_SAT_FOR_HUE_WARNING && darkHsl.s < MIN_SAT_FOR_HUE_WARNING) {
          return;
        }
        const drift = hueDelta(lightHsl.h, darkHsl.h);
        if (drift == null || drift <= MAX_HUE_DRIFT)
          return;
        warnings.push({
...
var __RECONCILE_DEBOUNCE_MS = 16;
var __MAX_RECONCILE_PASSES = 5;
var __MAX_INDEXED_VALUES = 1e3;
var __LOCALIZATION_STATE_KEY = "__PURE_DS_LOCALIZATION_STATE__";
function __createLocalizationState() {
  return {
    ...
    selfWrittenTextNodes: /* @__PURE__ */ new WeakSet(),
    textNodeValueMap: /* @__PURE__ */ new WeakMap(),
    attributeValueMap: /* @__PURE__ */ new WeakMap(),
    ...
    messageValueToKeys: /* @__PURE__ */ new Map(),
...
var __localizationState = __resolveLocalizationState();
...
function __invalidateValueCaches() {
  __localizationState.textNodeValueMap = /* @__PURE__ */ new WeakMap();
  __localizationState.attributeValueMap = /* @__PURE__ */ new WeakMap();
}
...
function __splitTextWhitespace(value) {
  ...
  const start2 = leading.length;
  const end = input.length - trailing.length;
  const core = end >= start2 ? input.slice(start2, end) : "";
...
function __findRequestedSubsegmentForText(coreText) {
  ...
  for (const [indexedValue, indexedKeys] of __localizationState.valueToKeys.entries()) {
    ...
    const start2 = coreText.indexOf(indexedValue);
    if (start2 === -1) {
      continue;
    }
    for (const key of indexedKeys) {
      ...
      const candidate = {
        ...
        start: start2,
        end: start2 + indexedValue.length,
        values
      };
...
async function __localizeRequestedTextNodes(roots) {
  if (typeof document === "undefined" || __localizationState.requestedKeys.size === 0 || typeof document.createTreeWalker !== "function") {
    return;
  }
  const nodes = [];
...
async function __reconcileLocalization() {
  ...
  const { roots, elements } = hasRequestedKeys ? __collectLocalizationRoots() : { roots: [], elements: [] };
...
var msg = (template, options = {}) => {
  ...
  if (isNewKey && !options?.element && !options?.scope && !options?.host && !options?.contextElement && !options?.lang) {
    __scheduleReconcile();
  }
  return translated;
};
...
var PDS2 = existingPDS && typeof existingPDS.addEventListener === "function" ? existingPDS : new PDSBase();
if (globalScope) {
  globalScope[PDS_SINGLETON_KEY] = PDS2;
}
if (typeof PDS2.log !== "function") {
  PDS2.log = (level = "log", message, ...data) => {
    if (typeof console === "undefined")
      return;
    const method = typeof console[level] === "function" ? console[level].bind(console) : typeof console.log === "function" ? console.log.bind(console) : null;
    if (!method)
...
if (typeof PDS2.logHandler !== "function") {
  PDS2.logHandler = null;
}
  ...
  async getStylesheet(layer) {
    if (this._mode === "live") {
      ...
      try {
        ...
        PDS2.log("error", `Registry: failed to load static ${layer}:`, error);
        PDS2.log("error", `Registry: looking for ${this._staticPaths[layer]}`);
        PDS2.log("error", "Registry: make sure you've run 'npm run pds:build' and configured PDS.start() with the correct static.root path");
...
function installRuntimeStyles(cssText) {
  try {
    ...
    PDS2.log("warn", "installRuntimeStyles failed:", err);
  }
}
...
var defaultPDSEnhancerMetadata = [
  ...
  {
    selector: "nav[data-dropdown]",
    description: "Enhances a nav element with data-dropdown to toggle its last child as a dropdown panel (menu, card, form, etc.). Add data-dropdown-close to any clickable descendant that should close the menu on selection.",
    attributes: [
      {
        name: "split-button",
        description: "Class that creates a split-button layout by mirroring the default menu item into a primary action button. Uses the first <li> by default, or <li data-default> when present.",
        appliesTo: "nav[data-dropdown]"
      },
      {
        name: "data-dropdown-close",
        description: "When clicked (or when a descendant is clicked), closes the currently open dropdown popover.",
  ...
  {
    selector: 'input[autocomplete="one-time-code"]',
    description: "Enhances a single text input into a segmented one-time-code / OTP field with numeric sanitizing, full-code paste support, and optional auto-submit when the expected length is reached.",
    attributes: [
      {
        name: "data-otp-length",
        description: "Expected code length. Defaults to the input maxlength, or 6 when neither is provided.",
        appliesTo: 'input[autocomplete="one-time-code"]'
      },
      {
        name: 'data-otp-autosubmit="false"',
        description: "Opt out of automatically submitting the parent form when the code is complete.",
        appliesTo: 'input[autocomplete="one-time-code"]'
      },
      {
        name: "data-otp-submit-selector",
        description: "Optional selector for the preferred submit button passed to form.requestSubmit().",
        appliesTo: 'input[autocomplete="one-time-code"]'
      }
    ],
    demoHtml: (
...
async function loadGoogleFont(fontFamily, options = {}) {
  ...
  if (existingLink) {
    PDS2.log("log", `Font "${primaryFont}" is already loading or loaded`);
    return Promise.resolve();
  }
  PDS2.log("log", `Loading font "${primaryFont}" from Google Fonts...`);
  return new Promise((resolve, reject) => {
    ...
    link.onload = () => {
      PDS2.log("log", `Successfully loaded font "${primaryFont}"`);
      resolve();
    };
    link.onerror = () => {
      PDS2.log("warn", `Failed to load font "${primaryFont}" from Google Fonts`);
      reject(new Error(`Failed to load font: ${primaryFont}`));
    };
    document.head.appendChild(link);
    setTimeout(() => {
      if (!isFontAvailable(primaryFont)) {
        PDS2.log("warn", `Font "${primaryFont}" did not load within timeout`);
      }
      resolve();
    }, 5e3);
  });
...
async function loadTypographyFonts(typographyConfig) {
  ...
  const loadPromises = Array.from(fontFamilies).map(
    (fontFamily) => loadGoogleFont(fontFamily).catch((err) => {
      PDS2.log("warn", `Could not load font: ${fontFamily}`, err);
    })
  );
...
function __slugify(str2 = "") {
  return String(str2).toLowerCase().replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
}
    ...
    const autoDefineConfig = {
      ...
      onError: (tag, err) => {
        if (typeof tag === "string" && tag.startsWith("pds-")) {
          ...
          PDS2.log("error", `\u274C Auto-define error for <${tag}>:`, err);
        }
      },
...
function cloneWithoutFunctions2(value) {
  const cloneCandidate = stripFunctions(value);
  if (typeof structuredClone === "function") {
    return structuredClone(cloneCandidate);
  }
...
function toReadonlyClone(value) {
  ...
  return deepFreeze(cloneWithoutFunctions2(value));
}
...
async function __attachLiveAPIs(PDS3, { applyResolvedTheme, setupSystemListenerIfNeeded, emitConfigChanged }) {
  if (__liveApiReady)
    return;
  const [ontologyModule, enumsModule, commonModule] = await Promise.all([
  ...
  if (!Array.isArray(PDS3.defaultEnhancers) || PDS3.defaultEnhancers.length === 0) {
    PDS3.defaultEnhancers = Array.isArray(defaultPDSEnhancers) ? defaultPDSEnhancers : [];
  }
  ...
  PDS3.applyLivePreset = async function(presetId, options = {}) {
    ...
    if (!PDS3.registry?.isLive) {
      PDS3.log("warn", "PDS.applyLivePreset is only available in live mode.");
      return false;
    }
    const baseConfig = PDS3.currentConfig || {};
    const { design: _design, preset: _preset, ...rest } = baseConfig;
    const inputConfig = {
      ...cloneWithoutFunctions2(rest),
      preset: presetId
    };
    ...
    if (persist && typeof window !== "undefined") {
      ...
      try {
        ...
        const nextStored = {
          ...
          design: cloneWithoutFunctions2(normalized.generatorConfig.design || {})
        };
  ...
  PDS3.preloadCritical = function(config, options = {}) {
    ...
    try {
      ...
      PDS3.log("warn", "PDS preload failed:", error);
    }
  };
  __liveApiReady = true;
}
async function startLive(PDS3, config, { emitReady, emitConfigChanged, applyResolvedTheme, setupSystemListenerIfNeeded }) {
  ...
  const explicitExternalIconPath = config?.design?.icons?.externalPath || config?.icons?.externalPath || null;
  config = applyStoredConfigOverrides(config);
  if (typeof explicitExternalIconPath === "string" && explicitExternalIconPath.trim()) {
    const baseDesign = config?.design && typeof config.design === "object" ? config.design : {};
    const baseIcons = baseDesign?.icons && typeof baseDesign.icons === "object" ? baseDesign.icons : {};
    config = {
      ...config,
      design: {
        ...baseDesign,
  ...
  if (typeof document !== "undefined" && document.adoptedStyleSheets) {
    ...
    try {
      ...
      PDS3.log("warn", "Constructable stylesheets not supported, using <style> tag fallback:", e);
  ...
  try {
    ...
    const generatorConfig = cloneWithoutFunctions2(configToClone);
...
async function getLiveTemplateById(templateId, options = {}) {
  ...
  if (!template)
    return null;
  const html2 = await readTemplateHtml(template, catalog);
  return { ...template, html: html2 };
}
...
// src/js/pds.js
if (typeof PDS2.initializing !== "boolean") {
  PDS2.initializing = false;
}
if (!("currentPreset" in PDS2)) {
  PDS2.currentPreset = null;
}
if (typeof PDS2.debug !== "boolean") {
  PDS2.debug = false;
}
if (!("currentConfig" in PDS2)) {
  PDS2.currentConfig = null;
}
if (!("compiled" in PDS2)) {
  PDS2.compiled = null;
}
...
function __getLocalizationRuntimeSync() {
  ...
  const sharedRuntime = PDS2?.[__LOCALIZATION_RUNTIME_SINGLETON_KEY__];
...
function __setLocalizationRuntime(runtime) {
  ...
  PDS2[__LOCALIZATION_RUNTIME_SINGLETON_KEY__] = normalizedRuntime;
}
configurePDSLogger({
  getLogger: () => typeof PDS2.logHandler === "function" ? PDS2.logHandler : null,
  getContext: () => {
    const mode = PDS2?.mode || PDS2?.compiled?.mode || (PDS2?.registry?.isLive ? "live" : "static");
    const debug = (PDS2?.debug || PDS2?.currentConfig?.debug || PDS2?.currentConfig?.design?.debug || PDS2?.compiled?.debug || PDS2?.compiled?.design?.debug || false) === true;
    return {
      mode,
      debug,
      thisArg: PDS2
...
function __replacePlaceholders2(input, callback) {
  return String(input).replace(
    /\{(\d+)\}/g,
    (_match, index) => callback(Number(index))
  );
}
function __normalizeMessages2(messages) {
  if (!messages || typeof messages !== "object") {
    return {};
  }
  const normalized = {};
...
function __fallbackMsg(template) {
  ...
  if (__isStrTagged2(template)) {
    const key2 = __collateStrings2(template.strings || []);
    const translated = __fallbackLocalizationState.messages[key2] || key2;
    return __replacePlaceholders2(translated, (index) => template.values?.[index]);
  }
  const key = String(template);
...
function __queuePendingLocalizationKey(template) {
  ...
  const key = __isStrTagged2(template) ? __collateStrings2(template.strings || []) : String(template);
  if (typeof key === "string" && key.length > 0) {
...
var configureLocalization = (config = null) => {
  ...
  if (Object.prototype.hasOwnProperty.call(config, "messages")) {
    __fallbackLocalizationState.messages = __normalizeMessages2(config.messages);
  }
...
function __resolveExternalRuntimeModuleURL(filename, overrideURL) {
  ...
  const assetRootURL = resolveRuntimeAssetRoot(PDS2.currentConfig || {}, {
    resolvePublicAssetURL
  });
  return `${assetRootURL}core/${filename}`;
}
async function __loadDefaultEnhancers() {
  if (Array.isArray(PDS2.defaultEnhancers) && PDS2.defaultEnhancers.length > 0) {
    return PDS2.defaultEnhancers;
  }
  if (!__defaultEnhancersPromise) {
    const enhancersModuleURL = __resolveExternalRuntimeModuleURL(
      "pds-enhancers.js",
      PDS2.currentConfig?.enhancersURL
    );
    __defaultEnhancersPromise = import(
      ...
      PDS2.defaultEnhancers = enhancers;
      return enhancers;
    }).catch((error) => {
...
async function __loadAsk() {
  if (typeof PDS2.ask === "function" && PDS2.ask !== __lazyAsk) {
    return PDS2.ask;
  }
  if (!__askPromise) {
    const askModuleURL = __resolveExternalRuntimeModuleURL(
      "pds-ask.js",
      PDS2.currentConfig?.askURL
    );
    __askPromise = import(
      /* @vite-ignore */
      askModuleURL
...
async function __loadToast() {
  if (typeof PDS2.toast === "function" && PDS2.toast !== __lazyToast) {
    return PDS2.toast;
  }
  if (!__toastPromise) {
    const toastModuleURL = __resolveExternalRuntimeModuleURL(
      "pds-toast.js",
      PDS2.currentConfig?.toastURL
    );
...
PDS2.registry = registry;
PDS2.enums = enums;
PDS2.adoptLayers = adoptLayers;
PDS2.adoptPrimitives = adoptPrimitives;
var parse = parseHTML;
PDS2.parse = parse;
var html = parseFragment;
PDS2.html = html;
PDS2.State = State;
PDS2.bindState = bindState;
PDS2.createStylesheet = createStylesheet;
PDS2.isLiveMode = () => registry.isLive;
PDS2.ask = __lazyAsk;
PDS2.toast = __lazyToast;
PDS2.common = common_exports;
PDS2.msg = msg2;
PDS2.str = str;
PDS2.configureLocalization = configureLocalization;
PDS2.loadLocale = loadLocale;
PDS2.setLocale = setLocale;
PDS2.getLocalizationState = getLocalizationState;
PDS2.createJSONLocalization = createJSONLocalization;
PDS2.i18n = {
  msg: msg2,
  str,
...
PDS2.AutoComplete = null;
PDS2.loadAutoComplete = async () => {
  if (PDS2.AutoComplete && typeof PDS2.AutoComplete.connect === "function") {
    return PDS2.AutoComplete;
  }
  const autoCompleteModuleURL = __resolveExternalRuntimeModuleURL(
    "pds-autocomplete.js",
    PDS2.currentConfig?.autoCompleteURL
  );
  if (!__autoCompletePromise) {
    __autoCompletePromise = import(
      ...
      PDS2.AutoComplete = autoCompleteCtor;
...
function __emitPDSConfigChanged(detail = {}) {
  ...
  try {
    ...
    PDS2.dispatchEvent(changedEvent);
  } catch (e) {
  }
  if (typeof document !== "undefined") {
    if (hasCustomEvent) {
...
var __themeDescriptor = Object.getOwnPropertyDescriptor(PDS2, "theme");
if (!__themeDescriptor) {
  Object.defineProperty(PDS2, "theme", {
    get() {
      try {
        if (typeof window === "undefined")
          return null;
    ...
    set(value) {
      try {
        if (typeof window === "undefined")
          return;
        const currentPreset = PDS2.currentConfig?.design || null;
        const resolvedTheme = resolveThemePreference(value);
        if (currentPreset && !isPresetThemeCompatible(currentPreset, resolvedTheme)) {
          const presetName = currentPreset?.name || PDS2.currentPreset?.name || PDS2.currentConfig?.preset || "current preset";
          PDS2.log(
            "warn",
            `PDS theme "${resolvedTheme}" not supported by preset "${presetName}".`
          );
...
PDS2.defaultEnhancers = [];
async function start(config) {
  PDS2.initializing = true;
  try {
    const mode = config && config.mode || "live";
    const { mode: _omit, ...rest } = config || {};
    PDS2.mode = mode;
    PDS2.logHandler = typeof rest?.log === "function" ? rest.log : null;
    PDS2.currentConfig = __toReadonlyClone(rest);
    ...
    if (mode === "static") {
      ...
      const { startLive: startLive2 } = await import(
        /* @vite-ignore */
        managerUrl
      );
      startResult = await startLive2(PDS2, managerConfig, {
    ...
    PDS2.compiled = __toReadonlyClone(startResult?.config || null);
    const resolvedExternalIconPath = PDS2?.compiled?.design?.icons?.externalPath || "/assets/img/icons/";
    PDS2.log("info", `startup ready; external icon path: ${resolvedExternalIconPath}`);
    return startResult;
  } finally {
    PDS2.initializing = false;
  }
...
async function staticInit(config) {
  ...
  try {
    ...
    PDS2.registry.setStaticMode(staticPaths);
    if (applyGlobalStyles && typeof document !== "undefined") {
      try {
        const stylesSheet = await PDS2.registry.getStylesheet("styles");
        if (stylesSheet) {
        ...
        __defaultLog.call(PDS2, "warn", "Failed to apply static styles:", e);
      }
    }
    let autoDefiner = null;
    let mergedEnhancers = [];
    try {
      const defaultPDSEnhancers2 = await __loadDefaultEnhancers();
      const res = await setupAutoDefinerAndEnhancers({
        autoDefineBaseURL,
        autoDefinePreload,
        autoDefineMapper,
        ...
        autoDefineOverrides: cfgAuto || null,
        autoDefinePreferModule: !(cfgAuto && cfgAuto.baseURL)
      }, { baseEnhancers: defaultPDSEnhancers2 });
      autoDefiner = res.autoDefiner;
      mergedEnhancers = res.mergedEnhancers || [];
    } catch (error) {
      __defaultLog.call(
    ...
    PDS2.compiled = __toReadonlyClone({
      mode: "static",
      ...resolvedConfig,
      theme: resolvedTheme,
      enhancers: mergedEnhancers
...
function injectImportStyleBlock(html2 = "", cssText = "") {
...
function extractTailwindRuntimeContext(sourceHtml = "", summary = null) {
  ...
  html2 = html2.replace(/<style([^>]*)>([\s\S]*?)<\/style>/gi, (fullMatch, attrs, cssBody) => {
...
function convertHtmlWithRules(html2 = "", options = {}) {
...
function inferPdsDesignFromHtml(input = {}) {
  ...
  const signals = collectHtmlSignals(html2);
  const state = {
    patch: {},
...
function convertTailwindHtmlToPds(input = {}) {
  const html2 = String(input.html || "").trim();
  if (!html2) {
    return createImportResult({
      source: "tailwind",
      type: "tailwind-html",
  ...
  const conversion = convertHtmlWithRules(html2, { config: input.config || {} });
  return createImportResult({
    source: "tailwind",
    type: "tailwind-html",
