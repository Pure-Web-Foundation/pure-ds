...
export class SvgIcon extends HTMLElement {
  ...
  static #fallbackIcons = {
    ...
    "magnifying-glass":
      '<path d="M229.66,218.34l-50.07-50.06a88.11,88.11,0,1,0-11.31,11.31l50.06,50.07a8,8,0,0,0,11.32-11.32ZM40,112a72,72,0,1,1,72,72A72.08,72.08,0,0,1,40,112Z" fill="currentColor"/>',
    ...
    "check-circle":
      '<path d="M173.66,98.34a8,8,0,0,1,0,11.32l-56,56a8,8,0,0,1-11.32,0l-24-24a8,8,0,0,1,11.32-11.32L112,148.69l50.34-50.35A8,8,0,0,1,173.66,98.34ZM232,128A104,104,0,1,1,128,24,104.11,104.11,0,0,1,232,128Zm-16,0a88,88,0,1,0-88,88A88.1,88.1,0,0,0,216,128Z" fill="currentColor"/>',
    ...
    missing:
      '<circle cx="128" cy="128" r="96" stroke="currentColor" fill="none" stroke-width="16"/>',
  };
  ...
  static #detectSpriteSupport() {
    ...
    const hasNoSpriteFlag = Boolean(
      docEl &&
      docEl.dataset &&
      Object.prototype.hasOwnProperty.call(docEl.dataset, "pdsNoSprite"),
    );
    ...
    try {
      ...
      const hasUseInterface =
        typeof window.SVGUseElement !== "undefined" &&
        use instanceof SVGUseElement;
      const hrefProperty = Boolean(
        use.href && typeof use.href.baseVal === "string",
      );
      const hrefAttribute =
  ...
  render() {
    ...
    if (
      ...
      const cacheKey = SvgIcon.getExternalIconCacheKey(
        icon,
    ...
    const resolveIconData = (iconName) => {
      let effectiveHrefLocal = spriteHref
        ? `${spriteHref}#${iconName}`
        : `#${iconName}`;
      ...
      let useFallbackLocal =
        this.hasAttribute("no-sprite") || !this.spriteAvailable();
      let spriteIconNotFoundLocal = false;
      let spriteStillLoadingLocal = false;
      const suppressDefaultMissingLocal =
        this.hasAttribute("data-icon-not-found") && iconName === attrIcon;
        ...
        try {
          ...
          if (inlineSpriteData && inlineSpriteData.loaded) {
            ...
            if (symbolData) {
              ...
              inlineSymbolPreserveAspectRatioLocal =
                symbolData.preserveAspectRatio;
            } else {
      ...
      if (
        spriteIconNotFoundLocal &&
        !hasFallbackLocal &&
        !spriteStillLoadingLocal &&
        !suppressDefaultMissingLocal
      ) {
        const cacheKey = SvgIcon.getExternalIconCacheKey(
          iconName,
          externalIconBasePath,
    ...
    if (
      ...
      !this._svgOldEl ||
      !this._svgNewEl ||
      !this._iconOldGroupEl ||
      !this._iconNewGroupEl
    ) {
      return;
    }
    ...
    this._stackEl.setAttribute(
      "class",
      `icon-stack${morphClass ? ` ${morphClass}` : ""}`,
    );
    ...
    if (currentData.preserveAspectRatio) {
      this._svgOldEl.setAttribute(
        "preserveAspectRatio",
        currentData.preserveAspectRatio,
      );
    } else {
    ...
    if (nextData.preserveAspectRatio) {
      this._svgNewEl.setAttribute(
        "preserveAspectRatio",
        nextData.preserveAspectRatio,
      );
    } else {
  ...
  #getMorphDurationMs() {
    try {
      const value = getComputedStyle(this)
        .getPropertyValue("--pds-icon-morph-duration")
        .trim();
      if (value) {
  ...
  static normalizeExternalIconPath(basePath) {
    const rawBasePath =
      typeof basePath === "string" ? basePath : SvgIcon.getExternalIconPath();
  ...
  static async fetchExternalIcon(iconName, basePath) {
    ...
    if (
      ...
      !SvgIcon.hasConfiguredExternalIconPath()
    ) {
      return false;
    }
    ...
    const promise = fetch(iconUrl)
      .then(async (response) => {
        ...
        if (!svg) {
          symbol =
            xmlDoc.querySelector("symbol") || htmlDoc.querySelector("symbol");
        }
        ...
        if (svg) {
          ...
          const appearsToBeSvgMarkup =
            /^<(path|g|defs|circle|ellipse|line|polyline|polygon|rect|use|mask|clipPath|linearGradient|radialGradient)\b/i.test(
              trimmed,
            );
          if (appearsToBeSvgMarkup) {
            ...
            throw new Error(
              `Invalid SVG payload from ${iconUrl}: no <svg>/<symbol> found (preview: ${preview})`,
            );
          }
        }
        ...
        if (!content) {
          throw new Error(
            `Invalid SVG payload from ${iconUrl}: empty icon markup`,
          );
        }
  ...
  static async ensureInlineSprite(spriteURL) {
    ...
    const promise = fetch(spriteURL, { mode: "cors" })
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(
            `Failed to load sprite: ${response.status} ${response.statusText}`,
          );
        }
        const svgText = await response.text();
        const parser = new DOMParser();
        ...
        symbols.forEach((symbol) => {
          ...
          const preserveAspectRatio = symbol.getAttribute(
            "preserveAspectRatio",
          );
        ...
        if (!symbolMap.size) {
          throw new Error(
            "Sprite does not contain any <symbol> definitions with valid ids",
          );
        }

        SvgIcon.inlineSprites.set(spriteURL, {
          loaded: true,
          error: false,
          symbols: symbolMap,
        });
